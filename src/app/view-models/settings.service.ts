...
export class SettingsService {
  ...
  setSelectedLang(lang: MLanguage): Observable<number> {
    ...
    return forkJoin([
      ...
      concatMap(res => {
        this.dictsReference = res[0] as MDictReference[];
        ...
        this.selectedDictTranslation = this.dictsTranslation.find(value => value.DICTID === this.USDICTNOTEID) ||
  ...
  private doUpdateUnitFrom(v: number, check: boolean = true): Observable<number> {
    ...
    this.USUNITFROM = v;
    return this.userSettingService.updateIntValue(this.INFO_USUNITFROM, this.USUNITFROM);
  }

  private doUpdatePartFrom(v: number, check: boolean = true): Observable<number> {
    if (check && this.USPARTFROM === v) return empty;
    this.USPARTFROM = v;
    return this.userSettingService.updateIntValue(this.INFO_USPARTFROM, this.USPARTFROM);
  }

  private doUpdateUnitTo(v: number, check: boolean = true): Observable<number> {
    if (check && this.USUNITTO === v) return empty;
    this.USUNITTO = v;
    return this.userSettingService.updateIntValue(this.INFO_USUNITTO, this.USUNITTO);
  }

  private doUpdatePartTo(v: number, check: boolean = true): Observable<number> {
    if (check && this.USPARTTO === v) return empty;
    this.USPARTTO = v;
    return this.userSettingService.updateIntValue(this.INFO_USPARTTO, this.USPARTTO);
  }
}
